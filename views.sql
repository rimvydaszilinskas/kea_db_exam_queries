...
CREATE VIEW language_movies
	AS SELECT languages.id, languages.name, COUNT(movies.id) AS movies 
    FROM languages 
    LEFT JOIN movies 
		ON movies.original_language=languages.id 
...
CREATE VIEW directors 
	AS SELECT celebrities.*, COUNT(directed.movie) AS movie_count 
	FROM celebrities 
	RIGHT JOIN directed ON directed.celebrity=celebrities.id GROUP BY celebrities.id;

SELECT * FROM directors;

DROP VIEW IF EXISTS actors;
CREATE VIEW actors
	AS SELECT celebrities.*, COUNT(acted.movie) AS movie_count 
	FROM celebrities 
	RIGHT JOIN acted 
		ON acted.celebrity=celebrities.id GROUP BY celebrities.id;
